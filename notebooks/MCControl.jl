...
# ╔═╡ 51bbe385-1729-4040-a440-b1878c8cb983
...
# ╔═╡ 7a3c59c8-6ca1-4986-9651-47b47957022c
...
# every-visit
function mcpolicyevaluation(π::Policy, k)::ActionValue
	γ = 1.0  # interesting predictable behaviour when γ not 1.0

	# Initialise the value function to zero. 
	q_π = zeros(length(𝒮), length(𝒜))
	q̂_π = zeros(length(𝒮), length(𝒜))
	g = zeros(length(𝒮), length(𝒜))

	n = zeros(length(𝒮), length(𝒜))

	num_iter = 0
	isevaluationconverged = false
	...
	@do_while begin
		...
		for t in length(episode):-1:1
			sₜ, aₜ, r = episode[t]
			gₜ = + r + γ * gₜ
			(si, ai) = (stateindex(sₜ, 𝒮), actionindex(aₜ, 𝒮))
			n[si, ai] += 1
			g[si, ai] += gₜ
		end
		q̂_π = g ./ n
	...
	# while ((num_iter < k) && ~(isevaluationconverged))
	# 	s = initialstate()
	# 	episode = createepisode(s, π)
	# 	gₜ = 0
	# 	for t in length(episode):-1:1
	...
	# 	end
	# 	q̂_π = g ./ n
	# 	isevaluationconverged = isconverged(q_π, q̂_π)
	# 	(isevaluationconverged) ? println("ActionValue Converged!! num_iter = ", num_iter) : nothing
	# 	q_π = q̂_π
	# 	num_iter += 1
	# end
...
begin
	...
	π₁ = createrandompolicy()
	episode = createepisode(s₁, π₁)
	q_π₁ = mcpolicyevaluation(π₁, 2000000)
end

# ╔═╡ f23067b8-5265-4cce-8e9e-7afa76a7f8b8
v_π₁ = maximum(q_π₁, dims=2)

# ╔═╡ 866703d9-3e9d-42e0-8141-d667ed29be3c
begin
	xs = zeros(length(𝒮))
	ys = zeros(length(𝒮))
	zs = zeros(length(𝒮))
	
	for si in 1:length(𝒮)
		s = 𝒮[si]
...
manifest_format = "2.0"
project_hash = "62538ac0c6e8f57dc819160a2760e6fa0088240d"
